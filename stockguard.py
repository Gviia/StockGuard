...
if st.button("분석 시작", type="primary"):
    if user_input:
        ...
        with st.status("🔍 분석을 시작합니다...", expanded=True) as status:
            ...
            6. 사용자 수준이 초보 (주린이, 용어 모름) 일경우, 학생도 이해할 수 있게 풀어서 설명하세요. 그리고 쉽게 풀어썼다는 것을 "쉬운 설명 : " 작성을 통해 강조하세요.
            """
...
if st.session_state.last_analysis:
    ...
    if st.button("🥺 내용이 어렵나요? 더 쉽게 설명해주세요", use_container_width=True):
        with st.status("✏️ 쉬운 설명으로 재작성 중...", expanded=True) as status_easy:
            ...
            easier_prompt = (
                "다음 내용을 사회 초년생도 이해하게 쉬운 비유로 설명해줘. "
                "예를 들어 영업이익 하락은 회사가 벌어들이는 돈이 줄다 이런식으로. : "
                + st.session_state.last_analysis
            )
            try:
